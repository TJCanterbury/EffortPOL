...
fn init_pop(n: usize, agent: Agent) -> Vec<Agent> {
...
struct Agent {
    ...
    x:f64, // Locus - mean POLS
    var:f64, // locus variance POLS
}
#[derive(Clone, Debug)]
struct Environment {
    pop: Vec<Agent>,
...
impl Agent {
    fn mutate(&mut self, mu:f64, mut_size:f64) {
        ...
        if dice <= mu { //mutates
            // loc3: f64, between 0. and 1. (lambda)
            // scales the perceived informativeness of partner gathering outcomes.
            let n = nm::new(self.x, mut_size as f64).unwrap(); // generates normal distribution 
            let loc_mut = n.sample(&mut rng);
            self.x = loc_mut;
        }
        ...
        if dice <= mu { //mutates
            // loc3: f64, between 0. and 1. (lambda)
            // scales the perceived informativeness of partner gathering outcomes.
            let n = nm::new(self.var, mut_size as f64).unwrap(); // generates normal distribution 
            let loc_mut = n.sample(&mut rng);
            if loc_mut < 0.0 {
                self.var = 0.0;
            } else {
    ...
    fn pols_det(&mut self) {
        let normal = Normal::new(self.x, self.var).unwrap();
    ...
    fn uncertainty(&self) -> f64 {
        if self.sigma > self.var {
            print!("\nsigma: {}, var: {}, pi: {}, x: {}", self.sigma, self.var, self.pi, self.x);
        }
        let h = self.sigma.powi(2) / self.var.powi(2);
        return h
    }
...
impl Environment {
            ...
            if male > female { // so that there aren't repeats
                ...
                // Record more data
                {
                    if (q1 > 0.) & (q2 < 0.) {
                        ...
                        self.mean_f_s_d += u1 / (u1+u2);
                    }
                    else if (q1 < 0.) & (q2 > 0.) {
                        dif += 1.;
                        self.mean_f_s_w += fitness;
                        self.mean_f_s_d += u2 / (u1+u2);
                    } else if (q1 > 0.) & (q2 > 0.) {
                        f_count += 1.;
                        self.f_w += fitness;
                    } else if (q1 < 0.) & (q2 <0.) {
                    ...
                    if u1 > u2 {
                        self.effort_dif += u1 - u2;
                        count+=1.;
                    } else {
                        self.effort_dif += u2 - u1;
                        count+=1.;
                    }
                }
            }
    ...
    fn pairing_pool(&mut self) { // divorces and then all attempt to pair
        ...
        for chunk in available.chunks(2) {
            if let [a, b] = chunk {
                // initialize states
                self.pop[*a].sigma = self.pop[*a].var;
                self.pop[*a].pi = self.pop[*a].x;
                self.pop[*a].partner = Some(*b);

                self.pop[*b].sigma = self.pop[*b].var;
    ...
    fn natural_selection(&mut self) {
        ...
        for i in 0..self.dead.len() {
            ...
            self.pop[self.dead[i]].x = blend*self.pop[selected_male].x + (1.-blend)*self.pop[selected_female].x;
            self.pop[self.dead[i]].var = blend*self.pop[selected_male].var + (1.-blend)*self.pop[selected_female].var;
    ...
    fn csvgen(&mut self, pop_len:f64, i:f64) -> Vec<f64> {
        ...
        mean_x = mean_x/pop_len;
        mean_var = mean_var/pop_len;
...
/////////////////////////////////////////// Initialise baseline parameters \\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\\
        ...
        let agent:Agent = Agent {
            ...
            x: 0.0, // Mean of POLS
            var: sigma0, // variance of POLS
        };
        
        let env = Environment {
            pop: init_pop(pop_size, agent),
...
////////////////////////////////////////////////////// Brood predation risk (b_p) sims
    ...
    (0..iterations).into_par_iter().for_each(|g|  {
        ...
        env.pop = init_pop(pop_size, agent);
